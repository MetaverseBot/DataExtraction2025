...
function requireFromWeb(moduleName) {
  const candidatePaths = [
    ...
    path.join(__dirname, "..", "node_modules", moduleName),
    path.join(__dirname, "..", "..", "web", "node_modules", moduleName),
  ];
  ...
  throw new Error(
    `Required dependency '${moduleName}' was not found. Expected one of: ${candidatePaths.join(", ")}`,
  );
}

const pdfParse = requireFromWeb("pdf-parse");
...
async function main() {
  ...
  try {
    console.log("Data Extraction (Deterministic Column Parser)");
    console.log("Input: Bank statement PDFs");
    console.log("Output: Name, Payment Date, Amount CSV");
    console.log("Rule: parse by PDF row/column positions, no merged-token inference.");
    console.log("");
    ...
    const outSummary = path.join(outputFolder, `extraction_summary_${stamp}.txt`);
    const summaryText = [
      "Data Extraction Deterministic Summary",
      `PDF files processed: ${pdfFiles.length}`,
      `Records extracted: ${allRecords.length}`,
      `Rejected lines: ${allInvalid.length}`,
      "",
      "Deterministic policy:",
      "- Date must be one MM/DD cell in row",
      "- Amount must be one currency cell in right-side amount column (x >= 420)",
      "- Description must contain Payment From",
      "- Name is parsed only from description text before trailing id token",
    ].join("\r\n");
    fs.writeFileSync(outSummary, `${summaryText}\r\n`, "utf8");
