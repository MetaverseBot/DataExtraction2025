...
async function main() {
  ...
  try {
    console.log("File Merge (Standalone EXE)");
    ...
    if (paymentRows.length === 0) {
      throw new Error("Donor file has no rows.");
    }
    if (dataRows.length === 0) {
      throw new Error("Extra data file has no rows.");
    }
    ...
    if (!paymentNameHeader || !dataNameHeader) {
      throw new Error("Both files must have a Name column.");
    }
    ...
    console.log("");
    console.log(`Done. Donor rows: ${paymentRows.length}`);
